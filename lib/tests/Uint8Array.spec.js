...
describe('Uint8Array', () => {
  ...
  it('Should have a predicate', () => {
    expect(TypeTags.isUint8Array).toBeDefined()
    expect(TypeTags.isUint8Array(uint8array)).toBeTrue()
    expect(TypeTags.isUint8Array(toString(uint8array))).toBeTrue()
    expect(TypeTags.isUint8Array(TypeTags.Uint8Array)).toBeTrue()
  })
})
