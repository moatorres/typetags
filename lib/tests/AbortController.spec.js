...
ifAvailable('AbortController', () => {
  ...
  it('Should have a predicate', () => {
    expect(TypeTags.isAbortController).toBeDefined()
    expect(TypeTags.isAbortController(abort)).toBeTrue()
    expect(TypeTags.isAbortController(toString(abort))).toBeTrue()
    expect(TypeTags.isAbortController(TypeTags.AbortController)).toBeTrue()
  })
})
...
ifAvailable('AbortSignal', () => {
  ...
  it('Should have a predicate', () => {
    expect(TypeTags.isAbortSignal).toBeDefined()
    expect(TypeTags.isAbortSignal(abort.signal)).toBeTrue()
    expect(TypeTags.isAbortSignal(toString(abort.signal))).toBeTrue()
    expect(TypeTags.isAbortSignal(TypeTags.AbortSignal)).toBeTrue()
  })
})
