...
describe('URL', () => {
  ...
  it('Should have a predicate', () => {
    expect(TypeTags.isURL).toBeDefined()
    expect(TypeTags.isURL(url)).toBeTrue()
    expect(TypeTags.isURL(toString(url))).toBeTrue()
    expect(TypeTags.isURL(TypeTags.URL)).toBeTrue()
  })
})
