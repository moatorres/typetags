...
describe('DataView', () => {
  ...
  it('Should have a predicate', () => {
    expect(TypeTags.isDataView).toBeDefined()
    expect(TypeTags.isDataView(view)).toBeTrue()
    expect(TypeTags.isDataView(toString(view))).toBeTrue()
    expect(TypeTags.isDataView(TypeTags.DataView)).toBeTrue()
  })
})
