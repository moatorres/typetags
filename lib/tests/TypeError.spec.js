...
describe('TypeError', () => {
  ...
  it('Should have a predicate', () => {
    expect(TypeTags.isTypeError).toBeDefined()
    expect(TypeTags.isTypeError(errCons)).toBeTrue()
    expect(TypeTags.isTypeError(toString(errCons))).toBeTrue()
    expect(TypeTags.isTypeError(TypeTags.TypeError)).toBeTrue()
  })
})
