...
describe('Uint32Array', () => {
  ...
  it('Should have a predicate', () => {
    expect(TypeTags.isUint32Array).toBeDefined()
    expect(TypeTags.isUint32Array(uint32array)).toBeTrue()
    expect(TypeTags.isUint32Array(toString(uint32array))).toBeTrue()
    expect(TypeTags.isUint32Array(TypeTags.Uint32Array)).toBeTrue()
  })
})
