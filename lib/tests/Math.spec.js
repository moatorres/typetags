...
describe('Math', () => {
  ...
  it('Should have a predicate', () => {
    expect(TypeTags.isMath).toBeDefined()
    expect(TypeTags.isMath(math)).toBeTrue()
    expect(TypeTags.isMath(toString(math))).toBeTrue()
    expect(TypeTags.isMath(TypeTags.Math)).toBeTrue()
  })
})
