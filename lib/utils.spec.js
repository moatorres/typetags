...
describe('hasToString', () => {
  it('Should be defined', () => {
    expect(utils.hasToString).toBeDefined()
  })

  it('Should allow us to check if value has a valid "toString" method', () => {
    const empty = Object.create(null)
    expect(utils.hasToString(0)).toBeTrue()
    expect(utils.hasToString({})).toBeTrue()
    expect(utils.hasToString('')).toBeTrue()
    expect(utils.hasToString(false)).toBeTrue()
    expect(utils.hasToString(empty)).toBeFalse()
  })
})
