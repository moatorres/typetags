...
export const TType = (type) => {
  ...
  const instance = (...args) => {
    ...
    if (isFactory()) return XType.of ? XType.of(...args) : XType(...args)
  ...
  const getTag = () => {
    ...
    if (type === 'NaN') return toString(NaN)
    if (type === 'Infinity') return toString(Infinity)
    if (type === 'AsyncFunction') return toString(argsMap.get(type))
